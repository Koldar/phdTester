...
class AbstractSpecificResearchFieldFactory(abc.ABC):
    ...
    def generate_curves_csvs(self,
                             ...
                             yaxis_name: str = None,
                             xaxis_name: str = None,
                             image_suffix: KS001 = None,
                             title: str = None,
                             subtitle_function: Optional["ISubtitleGenerator"] = None,
                             image_data_source: "IDataSource" = None,
                             image_dest_path: Optional["IDataContainerPathGenerator"] = None,
                             ):
        """
        ...
        :param ks001_to_add:
        :param use_format:
        :param csv_dest_data_source:
        :param csv_dest_path:
        :param path_function:
        :param mask_options:
        :param curve_changer:
        :param function_splitter:
        :param x_aggregator:
        :param csv_filter:
        :param data_source:
        :param skip_if_csv_already_exist:
        :param data_row_converter:
        :param generate_plots: if set to true for each csv generated we will create a plot representing it as well
        :param yaxis_name: use only if generate_plots if set to True. Set the y axis label
        :param xaxis_name: use only if generate_plots if set to True. Set the x axis label
        :param image_suffix: use only if generate_plots if set to True. Set the suffix the image generated will have
        :param title: use only if generate_plots if set to True. Se tthe title of the image
        :param subtitle_function: use only if generate_plots if set to True. A function which generates
            the subtitle of the image. If None we will generate the subtitle automatically
        :param image_data_source: use only if generate_plots if set to True. set the data source we will use to save
            the image we're produing. If left unset we will use `data_source`
        :param image_dest_path: use only if generate_plots if set to True. Set where in the datasource we will
            save the image. If left unset we will use `csv_dest_path`.
        :return:
        """
        ...
        for te in commons.distinct(map(lambda tc2: tc2.te, self.tests_repository)):
            ...
            if generate_plots:
                if image_data_source is None:
                    image_data_source = csv_dest_data_source
                if image_dest_path is None:
                    image_dest_path = FixedPathGenerator(csv_dest_path)
                ...
                self._print_images(
                    ...
                    title=title,
                    dest_datasource=image_data_source,
                    dest_path=image_dest_path,
                )
    ...
    def _generate_curves(self,
        ...
        if xstatus == XAxisStatus.UNKNOWN:
            ...
            raise ValueError(f"invalid x axis status {xstatus} (type {type(xstatus)})!")
    ...
    def _print_images(self,
                      ...
                      yaxis_name: str,
                      title: str,
                      image_suffix: Optional["KS001"] = None,
                      subtitle_function: "ISubtitleGenerator" = None,
                      dest_datasource: "IDataSource" = None,
                      dest_path: "IDataContainerPathGenerator" = None,
                      ):
        ...
        :param image_suffix: a KS001 instance that will be appended to the KS001 generated by the test context mask.
            If left unset we won't add any suffix to the image
        :param dest_datasource: the datasource where this image will be saved into. If absent we will use the filesystem
        :param dest_path: a function which will generate the path where the image will be saved into. If absent
            we will use "images".
        :return:
